...
use flourish::{
	...
	Guard, Propagation, SignalsRuntimeRef, ArcSubscription,
};
...
pub async fn skipped_while<'a, T: 'a + Send + Sync, SR: 'a + SignalsRuntimeRef>(
	...
	let sub = ArcSubscription::computed_with_runtime(fn_pin, runtime.clone());
	{
...
pub async fn filtered<'a, T: 'a + Send + Sync + Copy, SR: 'a + SignalsRuntimeRef>(
	...
	let sub = ArcSubscription::folded_with_runtime(
		MaybeUninit::uninit(),
		{
			shadow_clone!(once);
			move |value| {
...
>(
	sub: ArcSubscription<MaybeUninit<T>, S, SR>,
) -> ArcSubscription<T, impl Subscribable<T, SR>, SR> {
	#[pin_project]
	#[repr(transparent)]
	...
	unsafe {
		...
		(*(&(&ManuallyDrop::new(sub) as *const ManuallyDrop<ArcSubscription<MaybeUninit<T>, S, SR>>)
			as *const *const ManuallyDrop<ArcSubscription<MaybeUninit<T>, S, SR>>
			as *const *const ArcSubscription<T, AbiShim<S>, SR>))
			.read()
	}
}
