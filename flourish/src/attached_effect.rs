...
/// Type inference helper alias for [`AttachedEffectSR`] (using [`GlobalSignalRuntime`]).
pub type AttachedEffect<'a> = AttachedEffectSR<'a, GlobalSignalRuntime>;

/// An [`AttachedEffect`] ([`AttachedEffectSR`]) subscribes to signal sources just like a [`Subscription`](`crate::Subscription`) does,
...
///
/// The specified `drop` function also runs when the [`AttachedEffect`] is dropped.
#[must_use = "Effects are cancelled when dropped."]
pub struct AttachedEffectSR<'a, SR: 'a + ?Sized + SignalRuntimeRef = GlobalSignalRuntime> {
...
impl<'a, SR: SignalRuntimeRef> AttachedEffectSR<'a, SR> {
    pub fn new<T: 'a + Send>(
