...
impl<T: ?Sized> Guard<T> for ComputedGuardExclusive<'_, T> {}
...
impl<T: ?Sized> Deref for ComputedGuardExclusive<'_, T> {
	type Target = T;
...
impl<T: ?Sized> Borrow<T> for ComputedGuardExclusive<'_, T> {
	fn borrow(&self) -> &T {
		self.0.borrow()
	}
}
