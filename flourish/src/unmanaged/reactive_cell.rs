...
impl<T: ?Sized> Guard<T> for ReactiveCellGuard<'_, T> {}
impl<T: ?Sized> Guard<T> for ReactiveCellGuardExclusive<'_, T> {}

impl<T: ?Sized> Deref for ReactiveCellGuard<'_, T> {
	type Target = T;
...
impl<T: ?Sized> Deref for ReactiveCellGuardExclusive<'_, T> {
	type Target = T;
...
impl<T: ?Sized> Borrow<T> for ReactiveCellGuard<'_, T> {
	fn borrow(&self) -> &T {
		self.0.borrow()
	}
}

impl<T: ?Sized> Borrow<T> for ReactiveCellGuardExclusive<'_, T> {
	fn borrow(&self) -> &T {
		self.0.borrow()
	}
}
