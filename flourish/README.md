...
If you can't call `.get()` or `.set(…)` on pinned unmanaged signals, this import is what you're looking for:
...
// Note: `set`/`set_distinct` and other setters without suffix may be deferred (but are
//       safe to use in callbacks)!
//       Use the `…_blocking`, `…_eager` and `…_async` variants as needed.
a.set("a"); b.set("b"); // nothing
index.set_distinct(1); // "a" ("_distinct" methods don't replace or propagate if the value is equal)
a.set("aa"); // "aa"
b.set("bb"); // nothing
index.set(2); // "bb"
a.set("a"); // nothing
