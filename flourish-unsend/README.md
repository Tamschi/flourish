...
```rust
use flourish_unsend::{signals_helper, prelude::*, Propagation};
...
Additionally, inside `flourish_unsend::unmanaged`, you can find constructor functions for unpinned unmanaged signals that enable composition with data-inlining.
...
use flourish_unsend::{shadow_clone, LocalSignalsRuntime, Propagation};
...
|Dyn-compatibility for `trait Guard: Deref + Borrow<Self::Target> {}` as `dyn Guard<Target = …>` ([#40533](https://github.com/rust-lang/rust/issues/40533))|I think this is caused by use of the associated type as type parameter in any bound (of `Self` or an associated type). It works fine with `Guard<T>`, but that's not ideal since `Guard` is implicitly unique per implementing type (and having the extra generic type parameter complicates some other code).|
